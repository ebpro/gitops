...
- **Phase B** (A is done: r4 Complete 1/1, r1–r3 pruned):
  ...
  - Remaining: Gitea repo work (add `openapi.yaml` at repo root + contract step in `.woodpecker.yaml`) → Woodpecker secrets `MICROCKS_HOST/MICROCKS_CLIENT_ID/MICROCKS_CLIENT_SECRET` → sync `openapi.yaml` to GitHub copy + commit.
