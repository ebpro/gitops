...
- **Phase B** (A is done: r4 Complete 1/1, r1–r3 pruned):
  ...
  - Import proven via REST: `POST /api/artifact/upload?mainArtifact=true` → 201, service `link-shortener API:1.0.0-SNAPSHOT` (id `6a84bf1ecc8d8f29c60e656c`). CLI dead end documented: `quay.io/microcks/microcks-cli:1.13.2` doesn't exist (quay/Docker Hub publish nightly/sig tags only) — REST upload is the import path; `/api/artifact/download` (form `url=`) is the pipeline-friendly variant.
  - **RBAC gap found + fix committed**: Microcks 1.13.2 `SecurityConfiguration` is strict per-path, no hierarchy — `GET /api/services/{id}` and `/api/services/search` require `ROLE_user` even for admins (403 `insufficient_scope` with an admin-only grant; hit both the CI token and `platform-admins` UI users). Fix = cumulative role ladder: realm export (`platform-realm-configmap.yaml`) now defines the `microcks-app` client roles `admin/manager/user` (live UUIDs pinned) and group mappings are cumulative (admins: admin+manager+user; engineers/developers: manager+user; readonly: user). New Job `keycloak-microcks-roles-cumulative-20260818-r5` (`kubernetes/keycloak-realm/platform-realm-microcks-roles-job.yaml`) applies to the live realm (4 groups + `service-account-microcks-ci`) with per-target verification.
  - Remaining: r5 green → refetch CI token (expect roles `["admin","manager","user"]`) → detail/search 200 → rename service to `link-shortener` (set `info.title`, DELETE old, re-upload) → confirm mock answers → Gitea repo work (`openapi.yaml` + contract step in `.woodpecker.yaml`) → Woodpecker secrets `MICROCKS_HOST/MICROCKS_CLIENT_ID/MICROCKS_CLIENT_SECRET` → sync `openapi.yaml` to GitHub copy.

### Blocked
- Nothing hard. Open unknown: Woodpecker secret storage mechanism (Woodpecker DB vs Gitea repo secrets API) for the `MICROCKS_*` secrets.
