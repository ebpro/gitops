...
- **SonarQube 26 project/role provisioning (no local admin password)**: the SQ pod has no `SONAR_ADMIN_PASSWORD`; built-in `admin` works **only** via HTTP-header SSO on loopback from inside the pod (`sonar.web.sso.enable=true`, one header does it all: `Gap-Auth`): `kubectl exec -n sonarqube sonarqube-sonarqube-0 -c sonarqube -- sh -c 'curl -s -H "Gap-Auth: admin" http://localhost:9000/...'` — this is SonarQube's break-glass path. The 26.x API surface changed: create project `POST /api/projects/create` param is **`project`** (not `projectKey`); grant a user `POST /api/permissions/add_user` (`projectKey`, `login`, `permission`; project perms: `admin, codeviewer, issueadmin, securityhotspotadmin, scan, user`); `/api/user_roles/update` and `/api/users/who_am_i` are gone; full webapi index at `GET /api/webservices/list`. The 2026-09-18 full DB reset (fresh DB + fresh ES PVC) wiped ALL prior grants/tokens. Re-provisioned 2026-09-22 via the break-glass API: `bruno@ebruno.fr` is a member of the built-in global admin group **`sonar-administrators`** (= global admin; this 26.x instance has no `global-administrators` group — `GET /api/user_groups/search` shows only `sonar-administrators` + `sonar-users`), granted with `POST /api/user_groups/add_user?name=sonar-administrators&login=bruno%40ebruno.fr` (204; the 26.x group API takes `name`=group + `login`, not classic `group`+`logins`; list members via `GET /api/user_groups/users?name=sonar-administrators`); project `link-shortener` auto-registered post-reset; the Woodpecker `sonar-qa` token `ci` (USER_TOKEN, bruno, global scope) was recreated post-reset (2026-09-18). **A future DB reset wipes grants again** — re-grant with the same `user_groups/add_user` call. Verify grants in `sonardb`: `user_roles.role` joined via `user_uuid`→`users.login` and `entity_uuid`→`projects.uuid` (project key column is **`kee`**).
...
### ArgoCD OIDC SSO (Keycloak)
ArgoCD v3.5.1 logs in via Keycloak realm `platform`, client `argocd`. Two distinct failure modes were hit and fixed on 2026-09-12:

- **`unauthorized_client` "Invalid client or Invalid client credentials" (auth fails):** ArgoCD does NOT expand env vars in `oidc.config`. Use the `$secretName:key` reference: `clientSecret: $argocd-server-oidc-credentials:ARGOCD_SERVER_OIDC_CLIENT_SECRET` in `bootstrap/argocd-overlay/patches/argocd-cm.yaml`. The referenced Secret MUST be labeled `app.kubernetes.io/part-of=argocd` or ArgoCD's `partOfArgoCDSelector` informer never loads it (it then sends the literal `$...` string to Keycloak → 401). The Secret is ESO-managed (`kubernetes/postgresql/argocd-server-oidc-credentials.yaml`, Vault `secret/data/oidc/argocd` key `clientSecret`). **ESO v2.7.0 CRD PRUNES `spec.target.labels`** (`kubectl diff` → `Warning: unknown field "spec.target.labels"`); labels belong under `spec.target.template.metadata.labels`, NOT `spec.target.labels`. Wrong path → Secret silently unlabeled → informer never loads → 401. ArgoCD live-reloads labeled Secrets via informer (no restart). Verify: `kubectl get secret argocd-server-oidc-credentials -n argocd -o jsonpath='{.metadata.labels}'` shows `app.kubernetes.io/part-of=argocd`; `kubectl logs -n argocd deploy/argocd-server | grep 'secret key does not exist'` is empty.

- **Login succeeds but UI is empty — no apps/appsets (authorization fails):** ArgoCD RBAC (`bootstrap/argocd-overlay/patches/argocd-rbac-cm.yaml`) binds Keycloak groups → roles (`g, /platform-admins, role:admin`). Needs a `groups` claim in the id_token. Symptom: authenticated but zero resources (RBAC yields no role → empty list, NOT an error). Two sub-causes, both fixed 2026-09-12: (1) the realm `groups` client-scope had **no protocol mapper** — the realm JSON defines `oidc-group-membership-mapper` on it, but realm import is IGNORE_EXISTING so the pre-existing empty scope never got the mapper (fixed via one-shot Job `kubernetes/keycloak-realm/argocd-groups-mapper-job.yaml`, which POSTs the mapper via raw `/dev/tcp`). Config keys that matter: `claim.name=groups`, `full.group.path=true` (the repo previously had the wrong key `full.path`). (2) **Even with `full.group.path=true`, this Keycloak build emitted the BARE group name** (`platform-admins`, no leading slash), while the RBAC policy only bound `/platform-admins`. ArgoCD `g,` bindings are EXACT string match, so nothing matched. Fix: bind **both** `/x` and `x` forms in `argocd-rbac-cm` `policy.csv` (commit `78a3fcd`). **The user's ArgoCD session token must be freshly minted after the mapper exists** — a stale token has no `groups` key at all (log it via `kubectl logs -n argocd deploy/argocd-server | grep -o 'preferred_username[^}]*'` to see the exact claims ArgoCD received). A full logout+login (not just a page refresh) is required once the claim appears. Verify the claim independently by minting a password-grant token for the client (`directAccessGrantsEnabled=true` on argocd) and decoding `id_token.groups`.

- **kcadm gotchas in realm Jobs (Keycloak 26.7 image ships no curl/jq):** (1) `kcadm create .../protocol-mappers/models -s ...` fails with `Cannot parse the JSON [unknown_error]` — POST the mapper via raw `/dev/tcp` using the admin token extracted from `/opt/keycloak/.keycloak/kcadm.config` (pattern in `platform-realm-microcks-roles-job.yaml`). (2) `kcadm get client-scopes -q name=X` does NOT filter and the list INLINES protocol mappers (e.g. microprofile-jwt has a mapper literally named `groups`); to resolve a scope id match the object followed by `,"attributes"` (mappers are followed by `,"protocolMapper"`). (3) `UID` is a reserved readonly shell var — never use it for a Keycloak user id (use e.g. `USERID`).

- **Built-in roles only:** `role:admin` and `role:readonly` are ArgoCD built-ins (implicit grants). `role:projects-admin` is NOT built-in — it grants nothing until defined with explicit `p,` lines in `argocd-rbac-cm` `policy.csv`, so the `platform-engineers` binding is currently inert.
