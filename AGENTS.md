...
- **Login succeeds but UI is empty — no apps/appsets (authorization fails):** ArgoCD RBAC (`bootstrap/argocd-overlay/patches/argocd-rbac-cm.yaml`) binds Keycloak groups → roles (`g, /platform-admins, role:admin`). Needs a `groups` claim in the id_token. Symptom: authenticated but zero resources (RBAC yields no role → empty list, NOT an error). Root cause here: the realm `groups` client-scope had **no protocol mapper** — the realm JSON defines `oidc-group-membership-mapper` on it, but realm import is IGNORE_EXISTING so the pre-existing scope never got the mapper (fixed via one-shot Job `kubernetes/keycloak-realm/argocd-groups-mapper-job.yaml`). Config keys that matter: `claim.name=groups`, `full.group.path=true` (NOT `full.path` — the wrong key silently emits bare group names that won't match `/platform-admins`). **User must re-login** — an already-issued token won't carry the claim. Debug: decode the id_token, confirm `"groups":["/platform-admins",...]`.
