...
- **Pact broker HAL links re-host to public host (CI 401)**: The broker generates its HAL document links (e.g. `pb:provider-pacts-for-verification`) as absolute URLs from its configured `PACT_BROKER_BASE_URL` — not from the caller's URL. A CI client calling in-cluster still gets redirected to the configured host for the `for-verification` POST → 401 when that host sits behind `fwd-auth`. Keep `broker.config.baseUrl` in `helm/releases/pact-broker/values.yaml` as `http://pact-broker.pact-broker.svc:80` (in-cluster; chart 6.1.0 only exposes the singular env, no multi-base). Public `pactbroker.ebruno.fr` stays for browser SSO (relative UI links unaffected).
